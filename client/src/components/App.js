...
function get_std_graph_data(value_diffs, value_avg){
  ...
  for (const minute of Object.keys(value_diffs)){
    if (value_diffs[minute].length!==0){
      ...
      std_graph_data.low_conf.push(value_avg[i]-(std_dev/2));
...
function App() {
  ...
  function get_graphs_data(data){
    ...
    for(const x of dates){
      let minute = x.generation_date.getFullYear().toString()+"/"+("0" + x.generation_date.getMonth()).slice(-2).toString()+"/"+("0" + x.generation_date.getDate()).slice(-2).toString()+" "+
      ("0" + x.generation_date.getHours()).slice(-2).toString()+":"+("0" + x.generation_date.getMinutes()).slice(-2).toString(); 
      if (!group[minute]) {
        group[minute] = [];
      }
      group[minute].push(x);
  ...
  function onChangeDateRangeHandler(dataRangeValue){
    fetch("api?"+ new URLSearchParams({'start_date': dataRangeValue[0], 'end_date': dataRangeValue[1]})).then((res) => res.json()).then((data) => {get_graphs_data(data)})
  }

  React.useEffect(() => {
    fetch("/api?"+new URLSearchParams({'start_date': new Date('2023-03-28 15:00:00'), 'end_date':new Date('2023-03-29 10:17:00')})).then((res) => res.json()).then((data) => {get_graphs_data(data)})
  },[]);
  

  return (
    <div className="App">
      <Header onChangeDateRangeHandler={onChangeDateRangeHandler}></Header>
